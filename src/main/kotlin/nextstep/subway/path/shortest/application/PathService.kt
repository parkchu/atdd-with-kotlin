package nextstep.subway.path.shortest.application

import nextstep.subway.line.domain.LineStation
import nextstep.subway.line.domain.LineStationRepository
import nextstep.subway.path.fastest.domain.FastestPathApp
import nextstep.subway.path.shortest.domain.*
...
class PathService @Autowired constructor(
    ...
    fun findShortest(stationIds: List<Long>, type: String): PathResponse {
        ...
        val totalValue = TotalValue((path["총"]
                ?: error("")).map { it.toInt() }).get(type)
